...
// This file's manager and navigation fixtures describe activity on September 20.
// Keep delivered-photo ordering and lifecycle checks relative to that fixture day.
beforeEach(() => {
...
type GalleryModeLabel = 'Library' | 'Album' | 'Guest gallery';

/**
 * The switch carries each mode's count inside its own segment, so `Album` is named `Album0` and
 * `Guest gallery` can be named `Guest galleryOff`. The label is matched as a prefix, and inside the
 * switch, so the query stays on the segment instead of reaching `Album picks` or `Album details`.
 */
function galleryMode(label: GalleryModeLabel): HTMLElement {
  return within(screen.getByRole('group', { name: 'Gallery mode' }))
    .getByRole('button', { name: new RegExp(`^${label}`, 'u') });
}

/** The count each segment carries: the delivered total, the Album's photos, or `Off` for a Guest
 * gallery that is off. */
function galleryModeCount(label: GalleryModeLabel): HTMLElement {
  return galleryMode(label).querySelector<HTMLElement>('.gallery-mode-switch__count')!;
}

/** The same segment, for the tests that reach for it before the workspace has rendered. */
async function findGalleryMode(label: GalleryModeLabel): Promise<HTMLElement> {
  return within(await screen.findByRole('group', { name: 'Gallery mode' }))
    .getByRole('button', { name: new RegExp(`^${label}`, 'u') });
}

async function expandAlbumSettings(): Promise<void> {
  const disclosure = await screen.findByRole('button', { name: 'Album settings' });
  if (disclosure.getAttribute('aria-expanded') !== 'true') fireEvent.click(disclosure);
}

async function expandLibraryExports(user: ReturnType<typeof userEvent.setup>): Promise<void> {
  const summary = await screen.findByText('Exports', { selector: 'summary' });
  if (!summary.closest('details')?.hasAttribute('open')) await user.click(summary);
}

/**
 * The audience line is three labelled chips rather than one sentence. Reading them back as one
 * record keeps every fact in a single assertion: an event has two independent audiences, and a
 * check that quietly stopped covering one of them is exactly what this guards.
 */
async function galleryAudienceFacts(): Promise<Record<string, string>> {
  const audience = await waitFor(() => {
    const chips = document.querySelector<HTMLElement>('.gallery-audience');
    if (chips === null) throw new Error('The Gallery audience chips have not rendered.');
    return chips;
  });
  const facts = new Map<string, string>();
  for (const fact of audience.querySelectorAll('.gallery-audience__fact')) {
    facts.set(fact.querySelector('dt')!.textContent!, fact.querySelector('dd')!.textContent!);
  }
...
function managerFetch(pages: Record<string, MediaPage>, mediaRequests: string[] = []) {
  return vi.fn((input: RequestInfo | URL) => {
    const url = String(input);
    const parsed = new URL(url, 'https://candidary.test');
    ...
    if (parsed.pathname.endsWith('/gallery/arrivals')) {
      const afterSequence = Number(parsed.searchParams.get('after') ?? '0');
      return json({ afterSequence, snapshotSequence: 10, count: 0 });
    }
    if (url.includes('/media') || (url.includes('/gallery') && !url.endsWith('/gallery/summary'))) {
      ...
      return json(url.includes('/gallery') ? { ...page, snapshotSequence: 10, media: page.media.map((item, index) => {
        ...
        return { receivedAt, timelineAt: receivedAt, timelineSource: 'received',
          deliverySequence: index + 1, isFavorite: false, previewAvailable: true, ...row };
      }) } : page);
    }
...
function managerRotationFetch(options: {
  ...
  const fetchMock = vi.fn((input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (url.pathname === '/api/manage/events/event-a/gallery') {
      return json({
        media: makeMedia(2).slice(1).map((row, index) => ({ ...row, deliverySequence: index + 1 })),
        nextCursor: null,
        snapshotSequence: 10,
      });
    }
    if (url.pathname === '/api/manage/events/event-a/gallery/arrivals') {
      return json({
    ...
    if (url.pathname === '/api/manage/events/event-a/media/trash') {
...
function managerHistoryFetch(
  ...
  return vi.fn((input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (method === 'GET' && url.includes('/gallery?')) {
      ...
      return json({ media: library, nextCursor: null, snapshotSequence: 10 });
    }
...
describe('canonical Manager location ownership', () => {
  it.each([
    ...
    async (entry, sectionLabel, modeLabel, canonicalSearch) => {
      ...
      if (modeLabel) {
        expect(await findGalleryMode(modeLabel))
          .toHaveAttribute('aria-pressed', 'true');
      }
  ...
  it.each([
    ...
    async (requestSource, expectedSearch, expectedContent) => {
      ...
      expect(galleryMode('Album')).toHaveAttribute('aria-pressed', 'true');
      ...
      if (requestSource === 'Manager section') {
        ...
        expect(galleryMode('Library')).toHaveAttribute('aria-pressed', 'true');
      }
  ...
  it('captures a Library anchor before Guest gallery and restores it after Back', async () => {
    ...
    await user.click(galleryMode('Guest gallery'));
    await waitFor(() => expect(router.state.location.search)
  ...
  it('waits for a delayed Album return before restoring its Back anchor', async () => {
    ...
    expect(scrollTo).toHaveBeenCalledWith({ top: 780, behavior: 'instant' });
    expect(albumGets).toBe(1);
  });
  ...
  it('captures Library and Guest-gallery anchors before Back or Forward adopts another location', async () => {
    ...
    vi.spyOn(window, 'scrollY', 'get').mockImplementation(() => scrollY);
    trackWindowScroll((top) => { scrollY = top; });
    const libraryPath = `/manage/event/${MANAGED_EVENT.id}`;
    const guestPath = `/manage/event/${MANAGED_EVENT.id}?section=gallery&mode=guest-gallery`;
    const originalHistoryState = window.history.state;
    const originalHref = window.location.href;
  ...
  it('captures and consumes a distinct same-href browser POP exactly once', async () => {
    ...
    vi.spyOn(window, 'scrollY', 'get').mockImplementation(() => scrollY);
    trackWindowScroll((top) => { scrollY = top; });
    const managerPath = `/manage/event/${MANAGED_EVENT.id}`;
    const originalHistoryState = window.history.state;
    const originalHref = window.location.href;
    const targetWrapper = {
    ...
    window.history.pushState(currentWrapper, '', managerPath);
    window.dispatchEvent(new PopStateEvent('popstate', { state: currentWrapper }));
    ...
    const library = galleryMode('Library');
    library.focus();
    window.history.back();
    await waitFor(() => expect(router.state.location.state).toEqual({ source: 'task-4-test' }));
  ...
  it('uses a Share complete export intent once, focuses its enabled action, and preserves foreign state', async () => {
    ...
    galleryMode('Guest gallery').focus();
    await router.navigate(-1);
    await screen.findByRole('heading', { name: 'Share your event' });
    await router.navigate(1);
  ...
  it('cancels a successful held anchor capture after its Manager owner unmounts', async () => {
    ...
    fireEvent.click(await findGalleryMode('Guest gallery'));
  ...
  it('uses a retained marker intent once, focuses its first-page Restore, and does not replay on Back or reload', async () => {
    ...
    const trashGetsAfterIntent = trashGets;
    expect(trashGetsAfterIntent).toBeGreaterThanOrEqual(1);
  ...
  it('focuses a preloaded page-2 retained target without fetching another page', async () => {
    ...
    expect(screen.queryByText(/may be under Load more/u)).not.toBeInTheDocument();
    expect(requested.some((url) => url.includes('cursor=trash-page-2'))).toBe(true);
    expect(requested.some((url) => url.includes('cursor=trash-page-3'))).toBe(false);
  });
  ...
  it('omits retained Load-more guidance after the host exhausts the continuation', async () => {
    ...
    expect(heading).not.toHaveFocus();
    expect(screen.queryByText(/may be under Load more/u)).not.toBeInTheDocument();
    const continuationRequests = requested.filter((url) => url.includes('cursor='));
    expect(continuationRequests.length).toBeGreaterThan(0);
    expect(continuationRequests.every((url) => url.includes('cursor=exhausted-page-2'))).toBe(true);
  });
  ...
  it('abandons a delayed retained intent when the host manually leaves for Share', async () => {
    ...
    expect(screen.queryByText(/may be under Load more/u)).not.toBeInTheDocument();
    expect(trashGets).toBe(2); // Re-entering Trash reloads its page without replaying the retired focus intent.
  });
  ...
  it('Guest gallery Settings round trip cleans each intent, waits for save, and restores Hidden focus once', async () => {
    ...
    // Guest gallery picks the same way Library does: the checkboxes exist only while selecting.
    ...
    // The count lives in the tray, and an empty selection has no tray: its absence carries
    // exactly what the `0 of 50 selected` line used to.
    expect(screen.queryByRole('region', { name: 'Guest gallery' })).not.toBeInTheDocument();
    expect(screen.queryByText(`0 of ${MANAGER_BULK_SELECTION_MAX} selected`))
      .not.toBeInTheDocument();
    expect((router.state.location.state as RouterHistoryState)
...
describe('manager experience', () => {
  it('refreshes Photo Wall membership after immediate Undo under StrictMode', async () => {
    ...
    vi.stubGlobal('fetch', vi.fn((input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (path.endsWith('/gallery')) return json({
        media: [{ ...row, deliverySequence: 1, isFavorite: picked }],
        nextCursor: null,
        snapshotSequence: 1,
      });
      return base(input);
    }));
    render(<StrictMode><RouterProvider router={createAppRouter(['/manage/event/event-a'])} /></StrictMode>);
    const user = userEvent.setup();
  ...
  it('uses the event zone formatter for the Manager header, retention, and Intake schedule', async () => {
    ...
    try {
      ...
      const schedule = await screen.findByText(/Event start:/u);
  ...
  it('renders unavailable literals for invalid event zone values instead of normalizing them', async () => {
    ...
    await userEvent.setup().click(within(screen.getByRole('navigation', { name: 'Manager sections' }))
      .getByRole('button', { name: /settings/i }));
    expect(await screen.findByText(/Event start:/u)).toHaveTextContent(
  ...
  it('keeps appearance inside Settings between its form and account controls', async () => {
    ...
    const editor = await screen.findByRole('region', { name: 'Event appearance editor' });
  ...
  it('adopts the settings response without refreshing the whole manager', async () => {
    ...
    await user.click(await screen.findByLabelText('Show the optional shared gallery'));
  ...
  it('reconciles a confirmed galleryVisible change into an already-open Gallery summary', async () => {
    ...
    expect(galleryModeCount('Guest gallery')).toHaveTextContent(/^0$/u);
    expect(summaryReads).toBe(1);
    summaryVisible = false;
    releaseSettings();

    // The chip and the switch carry the same fact, so a confirmed change has to move both.
    await waitFor(() => expect(galleryModeCount('Guest gallery')).toHaveTextContent(/^Off$/u));
    expect(await galleryAudienceFacts()).toEqual({
      Album: '0 photos',
      'Album link': 'Off',
      'Guest gallery': 'Off, 0 published',
    });
  ...
  it('keeps the manager view in place when a bulk publish, hide, or export fails', async () => {
    ...
    await expectRecoverableFailure(
      'bulk publish',
      () => user.click(screen.getByRole('button', { name: 'Publish (1)' })),
      'Gallery',
    );
    await expectRecoverableFailure(
      'hide',
      () => user.click(screen.getByRole('button', { name: 'Hide (1)' })),
      'Gallery',
    );
    await user.click(galleryMode('Library'));
  ...
  it('reconciles another tab\'s active export after a stale Prepare conflict', async () => {
    ...
    await expandLibraryExports(user);
    expect(within(document.querySelector('.gallery-export-tools') as HTMLElement).getByText(
  ...
  it('adopts the newer export after a stale retry refusal without hiding the action error', async () => {
    ...
    await expandLibraryExports(user);
    const library = document.querySelector('.gallery-export-tools') as HTMLElement;
    expect(await within(library).findByText(/7 photos · Ready/, { selector: 'span' })).toBeVisible();
    expect(within(library).getByText('Frozen size: 7 KB · 0 guestbook entries.')).toBeVisible();
    expect(within(library).getByText('Ready')).toBeVisible();
  ...
  it('keeps the latest complete and album exports on their independent Gallery surfaces', async () => {
    ...
    await expandLibraryExports(user);
    const library = document.querySelector('.gallery-export-tools') as HTMLElement;
    expect(await within(library).findByText(/9 photos · Ready/, { selector: 'span' })).toBeVisible();
    expect(within(library).getByText('Frozen size: 1 KB · 3 guestbook entries.')).toBeVisible();
    expect(within(library).getByText('Ready')).toBeVisible();
    await user.click(galleryMode('Album'));
  ...
  it('posts the exact album kind selector from Download Album', async () => {
  ...
  it('caps cross-page bulk selection at 50 and submits only the selected ids', async () => {
    ...
    await user.click(screen.getByRole('button', { name: `Publish (${MANAGER_BULK_SELECTION_MAX})` }));
  ...
  it('does not reload the event entry when a Library query changes', async () => {
    ...
    vi.stubGlobal('fetch', vi.fn((input: RequestInfo | URL) => {
      ...
      if (url.endsWith('/gallery/summary')) return galleryAudienceSummaryJson();
      if (url.includes('/gallery?')) return json({
        media: makeMedia(2).slice(1).map((row, index) => ({ ...row, deliverySequence: index + 1 })),
        nextCursor: null,
        snapshotSequence: 10,
      });
  ...
  it('signs guest devices out without changing the printed event link', async () => {
    ...
    await user.click(await screen.findByRole('button', { name: 'Sign out guest devices' }));
  ...
  describe('safety ladder', () => {
    ...
    it.each(broadRows)(
      ...
      async (row) => {
        ...
        const trigger = await screen.findByRole('button', { name: ui.trigger });
  ...
  it('holds the rotate result behind copy, navigation, reload, and retired-resource gates', async () => {
    ...
    for (const path of [
      ...
      '/api/manage/events/event-a/media/trash',
      ...
      await waitFor(() => expect(calls.filter((call) => (
        call.method === 'GET' && call.path.split('?')[0] === path
      )).length, path)
        .toBeGreaterThanOrEqual(2));
    }
    expect(calls.filter((call) => (
      call.method === 'GET' && call.path.split('?')[0] === '/api/manage/events/event-a/gallery'
    ))).toHaveLength(1);
    await user.click(screen.getByRole('button', { name: 'Gallery' }));
    expect(await screen.findByRole('heading', { name: 'Library' })).toBeVisible();
    await waitFor(() => expect(calls.filter((call) => (
  ...
  it('opens on Library, searches private photos, and keeps the Guest gallery secondary', async () => {
    ...
    // `hidden` and would take the selection instead.
    const sharedWorkspace = await waitFor(() => {
      const workspace = document.querySelector<HTMLElement>('.gallery-shared-mode:not([hidden]) .gallery-shared');
      if (workspace === null) throw new Error('Guest gallery is not the active mode yet.');
      return workspace;
    });
    await user.click(within(sharedWorkspace).getByRole('button', { name: 'Select photos' }));
    await user.click(within(sharedWorkspace).getByRole('checkbox', { name: /The toast/i }));
  ...
  it('keeps Shared publication separate, clears filters, and scopes its busy and preview states', async () => {
    ...
    await user.click(within(shared).getByRole('button', { name: 'Select photos' }));
    ...
    // Both verbs live in the selection tray now, and an empty selection has no tray to offer
    // them from — the same guarantee the two disabled buttons used to carry.
    expect(within(shared).queryByRole('region', { name: 'Guest gallery' })).not.toBeInTheDocument();
    ...
    await user.click(within(shared).getByRole('button', { name: 'Publish (1)' }));
    const publishing = within(shared).getByRole('button', { name: 'Publishing…' });
    expect(publishing).toBeDisabled();
    expect(within(shared).getByRole('button', { name: 'Hide (1)' })).toBeDisabled();
    expect(within(shared).queryByRole('button', { name: 'Hiding…' })).not.toBeInTheDocument();
    // `aria-busy` belongs to the tray rather than to each verb: one write is running, and the
    // verb that started it says so in its own label.
    const tray = within(shared).getByRole('region', { name: 'Guest gallery' });
    ...
    expect(Array.from(document.querySelectorAll<HTMLElement>(
      '[data-gallery-live-host] [role="status"]',
    )).filter((status) => status.textContent?.trim())).toHaveLength(1);
  ...
  it('preserves the selected Gallery mode while a Manager epoch reloads Shared and refreshes Library on return', async () => {
    let libraryReads = 0;
    let sharedReads = 0;
    vi.stubGlobal('fetch', vi.fn((input: RequestInfo | URL) => {
      const url = String(input);
      if (url.includes('/gallery?')) {
        libraryReads += 1;
        return json({ media: [], nextCursor: null, snapshotSequence: 10 });
      }
      if (url.includes('/media')) {
        sharedReads += 1;
    ...
    expect(galleryMode('Guest gallery')).toHaveAttribute('aria-pressed', 'true');
    await waitFor(() => expect(sharedReads).toBe(2));
  ...
  it('adopts each confirmed mixed-status Shared bulk group and retries only the failed group', async () => {
    ...
    await user.click(within(shared).getByRole('button', { name: 'Publish (2)' }));
  ...
  it('offers recovery and discards only to the exact current Router destination after an Album conflict', async () => {
    ...
    expect(galleryMode('Library')).toBeDisabled();
    expect(galleryMode('Guest gallery')).toBeDisabled();
    ...
    expect(galleryMode('Library')).toBeEnabled();

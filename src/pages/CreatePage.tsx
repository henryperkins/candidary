...
export function CreatePage() {
  ...
  return <div className="public-shell"><PageHeader action={<Link className="text-link" to="/">Back home</Link>} /><main className="create-layout">
    ...
    <form className="create-form" ref={formRef} onSubmit={submit} noValidate>
      ...
      <label>Event name<input name="name" maxLength={80} required aria-invalid={Boolean(fields.name)} aria-describedby={fields.name ? 'name-error' : undefined} />{fields.name && <small id="name-error">{fields.name}</small>}</label>
      <label>Event date<input name="eventDate" type="date" required aria-invalid={Boolean(fields.eventDate)} aria-describedby={fields.eventDate ? 'eventDate-error' : undefined} />{fields.eventDate && <small id="eventDate-error">{fields.eventDate}</small>}</label>
      <label>Welcome message<textarea name="welcomeMessage" rows={4} maxLength={500} required placeholder="Tell guests what you’d love them to share." aria-invalid={Boolean(fields.welcomeMessage)} aria-describedby={fields.welcomeMessage ? 'welcomeMessage-error' : undefined} />{fields.welcomeMessage && <small id="welcomeMessage-error">{fields.welcomeMessage}</small>}</label>

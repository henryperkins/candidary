...
**Architecture:** `GuestUploadFlow` gains a `manager` variant through props and slots; it is not copied. The queue gains exactly two extensions — a `mediaId` on `ReservationResult` and an `onFinalized` per-item signal — because per-item invalidation is the smallest missing thing, not a second receipt system. A pure `manager-upload-cleanup.ts` controller owns the ambiguous-commit resolution so its race behavior is unit-testable without a DOM. The dialog reuses the tested `GalleryViewer` modal contract rather than inventing a Manager dialog framework.
...
- **Depends on** `2026-08-27-host-gallery-manager-upload-authority.md`. The four Manager routes, `UploadAuthority`, and migration `0021` must already be green. Do not begin until they are.
...
### Task 2: The two queue extensions
...
- the same race where the replay returns a canonical deleted/canceled result settles without another cancel;
...
```bash
git add src/features/uploads/GuestUploadFlow.tsx src/features/uploads/ManagerUploadDialog.tsx tests/ui/manager-upload-dialog.test.tsx tests/ui/guest-upload-flow.test.tsx
...
- the toolbar action is present whenever Intake is active, including when the event is paused;
...
Complete C-12's row: name the dialog, the two queue extensions, the cleanup controller's exact race branches, the always-present trigger, and the owning test files. Extend C-08's row with the client half — the Intake trigger stays available while guest delivery is paused. Do not claim guest-surface parity, first-run copy, Album, registration, or rotation behavior here; those belong to later checkpoints.

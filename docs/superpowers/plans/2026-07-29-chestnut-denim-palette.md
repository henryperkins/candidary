...
project's `tokens/colors.css`), on the host's explicit direction — no purple in global Candidary or
the Candidary Default event appearance. The three separately named curated event presets retain
their approved preset-owned focus colors.
...
was hue-rotated to the chestnut family (hue ≈ 17°) at equal saturation and lightness, preserving the
intended neutral/lightness hierarchy. Hue rotation can still change relative luminance, so final
text, control, and focus ratios are remeasured by the unit and browser contrast gates rather than
...
`#d98b6a` end stop; only the two purple stops moved. The Candidary Default no-cover top overlay is
the one alpha exception: it increases from 8% to 10% so the integrated Chestnut/Denim gradient
continues to clear the 4.5:1 white-copy floor at every tested geometry.
...
2. **`shared/event-theme.ts` `candidary-default` preset** — remap with the tables above:
   ...
   overlays/scrim `rgb(31 9 28 / *)` → `rgb(31 15 9 / *)`, with `heroOverlayTop` raised from 8% to
   10% for the tested white-copy floor, `fullscreenBackdrop #170e0a`,

...
The old scheduled export cleanup is a different control flow: it deletes Ready artifacts before attempting `markExpired()`, so a D1 trigger cannot make the full old Worker safe for an attempt-v2 Ready row. The isolated preview cuts over first: freeze uploads, apply 0020, upload and inspect an exact inert preview version, drain legacy work, close, prove the full preview config has no route/Cron/queue/event/address side effects, promote that version, update and inspect its Workflows, verify safe 503 while closed, then open with its exact UUID. Failure leaves preview admission non-open and hosted export conformance unavailable until a reviewed forward fix succeeds.

For production, the release owner first proves the sole active 100% Worker tag is the exact frozen source `df2b66510ccee6893ca91ab752337df8e52c6207`; any mismatch stops the release until the actual deployed source is frozen and reviewed. The owner blocks production deploys and unrelated `main` merges, switches the connected Build to the existing helper's upload-only mode, applies 0020 in `legacy-open`, and captures the reviewed merged version as an inert full-SHA-tagged artifact. The existing helper projects a Cron-only config and a Workflow-only config from that validated artifact. The Cron-only config detaches daily lifecycle while retaining hourly maintenance without PUTting Workflow definitions; the owner records the detach and drains through trigger propagation plus maximum invocation time. After every legacy queued/running job becomes terminal, one D1 statement atomically closes admission. Old active writes then lose. The inert Worker is immediately deployed alone at 100%; with new HTTP returning safe 503, the Workflow-only config updates the exact three production Workflow definitions without mutating Cron, routes, queues, or event triggers. Only after status and Workflow inspection agree does one D1 update open admission with that exact lowercase UUID active version and canonical timestamps. Both Crons are then restored with the Cron-only config. The freezes remain until a later daily `cleanup_completed` record names the exact cron and active Worker version; hourly evidence is insufficient. Frozen-old-cleanup tests prove why this gate is required; they do not pretend the deletion can be rolled back. Canonical commands live only in `docs/deployment.md`.

Closing is the one-way export-availability point: `legacy-open` cannot be restored, so any failure after
close must move forward to compatible HTTP/Workflow ownership. Promotion remains the distinct
trash/data rollback point because trash can be accepted immediately even while export admission is
closed. From promotion onward, the 0018 artifact cannot restore trash and cannot safely own scheduled

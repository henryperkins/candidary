...
3. `0021_manager_upload_and_album_era.sql`: server-only account upload-actor identity, deterministic live-Manager-token normalization plus the one-live-token partial unique index, the account-authorized Manager-link revision, the durable per-pick provenance marker, and an event-owned Album-pick generation.

Each runs against a fresh D1 database and its real populated predecessor fixture. In particular, 0021 upgrades populated 0020 state, normalizes duplicate live Manager tokens, and proves the still-serving 0020 Worker cannot create a second live Manager token after the new partial unique index exists. No existing migration is edited.

Slice 5 is implemented as one reviewed change set: strict focused RED, minimal implementation, and focused GREEN per independently testable task; a fresh implementer handoff and independent task review gate per task; then final Slice review and exactly one Slice commit. Its plans contain no task-level commits and no repository-wide test, build, lint, typecheck, E2E, or migration gates. Slice 5 verification is limited to named test files/spec filters, changed-file lint, the matrix parser, and `git diff --check`; it never pushes, merges, publishes, deploys, or runs a migration against shared/external state. Slice 6 findings C-34, C-38, and C-62 remain untouched.

Migration 0019 deliberately refuses to run while any export job is `running`; deployment waits for the existing Workflow to become terminal and retries the all-or-nothing migration. Migration-first compatibility is required while the 0018 Worker is still serving. Because its scheduled Ready cleanup deletes R2 before its D1 transition, the exceptional release blocks every production deploy and all `main` merges except its exact reviewed release before switching the existing connected production Build to provenance-checked upload-only mode. That one excepted merge produces an inert preflight version whose full-SHA tag and ID are verified while the deploy and exception-scoped merge freezes remain in force. The owner generates a Cron-only config and a full cutover config whose only production delta is `triggers.crons: []`, records all three current Workflow version IDs, detaches only the daily lifecycle Cron, and observes Cloudflare's complete trigger-propagation plus invocation wall-time drain. After D1 admission closes, the owner uses pinned `wrangler deploy` from the same clean exact-SHA artifact and full no-Cron cutover config. Before admission opens, production must show one new sole 100% active Worker with the exact SHA tag and all three latest Workflow version IDs changed from their baselines. Both Crons are restored only after the row opens; the branch remains frozen and Build upload-only until a later daily-lifecycle cleanup record carries the exact Cron expression and new active version ID. A reviewed forward fix may replace the expected version under that same owner/window, so the proof gate cannot prohibit its own recovery. Only after proof is the routine deploy command restored without replaying the already-handled Build. Pinned old Workflow callbacks remain protected by the database ownership fence. After the new Worker admits the first trash write or `attempt-v2` export, the release is forward-fix-only: the standard 0018 code rollback is no longer a valid recovery path. The verification suite distinguishes callback safety, the forbidden old cleanup path, and current-worker forward repair/hold release.

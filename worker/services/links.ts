...
export class LinkService {
  ...
  async rotate(auth: AuthenticatedSession, role: Role, now = new Date()) {
    const tokens = new TokensRepository(this.env.DB);
    await tokens.revokeRole(auth.event.id, role, now.toISOString());
    ...
    await tokens.create({
      ...
      expiresAt: role === 'guest'
        ? auth.event.guestAccessExpiresAt
        : auth.event.managementAccessExpiresAt,

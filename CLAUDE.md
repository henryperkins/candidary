...
Local secrets go in `.dev.vars` (copy `.dev.vars.example`). `TOKEN_HMAC_KEY`, `SESSION_HMAC_KEY`, and
`GUEST_TOKEN_ENCRYPTION_KEY` must be three independent values; the last must be exactly 32 bytes base64url.

## Architecture

One Cloudflare Worker (`worker/index.ts`) serves everything: a Hono API, the React SPA via the `ASSETS`
binding, a `scheduled` cleanup handler, and the exported `ExportWorkflow` class. Bindings are `DB` (D1),
`MEDIA_BUCKET` (private R2), `IMAGES`, `EXPORT_WORKFLOW`, and `ASSETS`.

Two TypeScript projects share one repo: `tsconfig.app.json` (`src`, `tests/unit`, `tests/ui`) and
`tsconfig.worker.json` (`worker`, `tests/worker`). Both include `shared/`, which is imported by relative
path (`../../shared/constants`) from either side — there are no path aliases. `tests/e2e`, `scripts/`, and
`playwright.config.ts` belong to neither project, so `npm run typecheck` does not cover them.

### Authorization

Ownership is link-bearer, with optional host accounts as a way back. A link is `id.secret`; D1 stores
only a keyed HMAC digest of the secret (`worker/security/crypto.ts`). An account
(`migrations/0006_host_accounts.sql`, `worker/routes/host-auth.ts`) only lets a host find their events
again without the link — it is never the ownership model, and no screen may imply the management link
stops working or that an account is required to run an event. There is no guest account.
...
Every API handler independently calls `AuthService.resolve(getSessionCookie(context))` and then compares
the authenticated event/role against the path parameters — there is no auth middleware, and route
identifiers alone never grant access. Writes additionally call `assertCsrf`, which checks `Origin` against
`APP_ORIGIN` and matches the CSRF cookie, header, and stored digest. Follow this per-route pattern
(`guestForSlug` in `routes/uploads.ts`, `managerForEvent` in `routes/manage.ts`) for new endpoints.
...
and purges retention-due events.
...
- **Limits** live in `shared/constants.ts` (20 MB/photo, 10,000 photos, 100 GiB/event, batch of 20).
  Treat that file as the source of truth — prose in `docs/security.md` still quotes superseded numbers.
- **New image format**: update `SUPPORTED_IMAGE_TYPES`, the client `accept`/validation sets in
  ...
  constraint.
- **New migration**: also append the filename to the array in `vitest.worker.config.ts`; worker tests
  apply migrations from that hardcoded list via `TEST_MIGRATION_QUERIES`, not from `migrations_dir`.
...
ten-point wedding rehearsal release gate.

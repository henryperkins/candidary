...
| Six manager destinations | RSVP inserted after Intake at every width | `manager-nav-768.png`, `manager-nav-count-390.png`, `manager-responsive.spec.ts`, `rsvp-responsive.spec.ts` | Every label stays visible at the 14 px control-text floor from 320 px upward, the count badges do not collide with them, and the vertical rail stays packed at the top. Below 761 the six destinations sit at the foot of the viewport rather than the top, and the badge is a dot whose figure is carried by the destination's accessible name — carried here by `manager-responsive.spec.ts`, which asserts both on each side of the rail. `manager-nav-count-390.png` is evidence for the 768 px rail only until its recapture lands; it still pictures the previous stacked header. |
